...
In addition, the general module
...
For upgrading to the latest version, type

>>> pip install --upgrade --no-deps scikit-kinematics

**Warning**: Do **not** use ``pip install scikit-kinematics -U``, since
that command also upgrades dependencies such as numpy.  This can break e.g.
